...
int largo(Nodo *arbol) {
    if (arbol == NULL)
        return 0;
    else
        return 1 + largo(arbol->getIzquierda()) + largo(arbol->getDerecha());
}

void profundidad(Nodo* arbol,int *hojas) {
	if (arbol == NULL) {
		return;
	}
	...
	// Juzgando el sub�rbol izquierdo
	profundidad(arbol->getIzquierda(),hojas);
	// Juzgando el sub�rbol correcto
	profundidad(arbol->getDerecha(),hojas);
}


int altura(Nodo* _arbol)
{
    ...
    return 1 + max(altura(_arbol->getIzquierda()), altura(_arbol->getDerecha()));
}
